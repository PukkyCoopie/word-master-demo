...
export const PRIVACY_POLICY_BODY_BLOCKS = [
  ...
  ul(
    ...
    [
      b("TapTap 云存档数据"),
      t("：当您登录 TapTap 账号并使用本游戏时，我们会将您的游戏进度（含存档槽位、生涯统计与游戏设置）打包上传至 TapTap 云存档服务器，并与您的 TapTap 账号标识（如 unionId）绑定，用于跨设备恢复进度。若本机与云端均有存档，您可在登录后选择保留哪一份。未登录 TapTap 时，进度仍仅保存在本机，不会上传云存档。"),
      b("收集方式"),
      t("为您主动登录 TapTap 后，由本应用调用 TapTap 云存档 SDK 上传；"),
      b("使用目的"),
      t("保存与恢复游戏进度、换机续玩；"),
      b("收集范围"),
      t("游戏存档 bundle 及 TapTap 账号关联信息，不含与本游戏无关的个人资料。"),
    ],
    [
      b("TapTap 排行榜数据"),
      t("：当您登录 TapTap 账号并在游戏中达成相应条件时，我们会通过 TapTap 排行榜 SDK 向 TapTap 上报与排名相关的"),
      b("游戏成绩数值"),
      t("，例如无尽模式大关进度、单次拼词最高得分、已达成难度序号、整局单词平均长度（四舍五入取整）等。上述数据与您的 TapTap 账号绑定，并可能在 TapTap 公开排行榜中展示；排行榜页面的昵称、头像等展示信息由 TapTap 按账号资料提供。我们"),
      b("不会"),
      t("向 TapTap 排行榜接口上传您拼出的具体单词文本。"),
      b("收集方式"),
      t("为您登录 TapTap 后，由本应用在达成上报条件时自动调用 TapTap 排行榜 SDK 提交；"),
      b("使用目的"),
      t("参与 TapTap 游戏排行榜、展示排名与竞技记录；"),
      b("收集范围"),
      t("排行榜 ID、对应数值分数及 TapTap 账号关联信息。"),
    ],
  ),
  h("（二）设备权限调用情况"),
  p([
    t("在您使用单词大师过程中，我们可能需要在必要范围内声明或调用设备权限。TapTap 云存档、排行榜等功能需使用"),
    b("网络访问"),
    t("与"),
    b("网络状态"),
    t("权限以上传、下载数据及提交排行榜分数；上述权限为普通权限，安装应用时自动授予，不会单独弹出授权窗口。请您知悉，我们不会默认开启您设备中需手动确认的危险权限（如相机、存储、定位等）；如您在首次授权开启某权限后希望关闭，可在设备的设置功能中选择关闭。"),
  ]),
  h("（三）我们如何使用您的个人信息"),
  p([
  ...
  p([
    t("我们不会向第三方出售您的个人信息。除本政策及法律法规另有规定外，我们不会共享、转让您的个人信息。您参与 TapTap 公开排行榜时，与排名相关的游戏成绩及 TapTap 账号展示信息（如昵称、头像）将按 TapTap 排行榜规则向其他用户公开显示，此属于您使用排行榜功能所产生的公开披露。"),
  ]),
  gap(),
  ...
  h("九、本隐私政策如何更新"),
  p([
    t("我们的隐私政策可能会适时发生变更。我们会在应用内发布对本隐私政策所做的任何变更。对于重大变更，我们还会提供更为显著的通知（我们可能会通过在应用内做特别提示等方式，说明隐私政策的具体变更内容）。未经您明确同意，我们不会削减您按照本隐私政策所应享有的权利。"),
  ]),
  gap(),
  h("十、如何联系我们"),
  p([
