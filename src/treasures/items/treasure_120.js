...
export default {
  ...
  description: describe(
    "每个关卡的第一次丢弃如果仅弃掉了1个字母，从字母库中移除它以获得",
    money("3"),
  ),
};
 ...
 * @returns {{ proc: false } | { proc: true, deckUid: number, moneyAmount: number, slotIndex: number }}
 */
export function resolvePistolDiscardBatchPlan(ctx) {
  ...
  const slotIndex = slots.findIndex((tid) => String(tid ?? "") === ID);
  if (slotIndex < 0 || ctx.letterCount !== 1) return { proc: false };
  const uid = ctx.discardedDeckCardUids?.[0];
  if (uid == null) return { proc: false };
  return { proc: true, deckUid: uid, moneyAmount: MONEY_AWARD, slotIndex };
}
...
export const treasureHooks = {
  async onDiscardBatch(ctx) {
    ...
    await ctx.playOwnedTreasureMoneyFx?.(ID, MONEY_AWARD, slotIndex != null ? { slotIndex } : {});
  },
};
