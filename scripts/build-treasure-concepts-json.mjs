...
const USER_BATCH_50 = [
  ...
  { batchIndex: 17, text: "$7 common 每当一个字母被加入你的字母库，获得x0.25倍率（当前x1）" },
  ...
  {
    batchIndex: 36,
    text: "$6 rare 每当进入关卡时，在你的字母库中添加一个具有随机增益效果的字母并立即抽到它【joker会在tile掉落前wobble出一个tile，然后tile飞入字母库动画、然后才是grid中的tile从上方掉下来的动画，且抽取时必定抽到这个字母】【解锁前提：拥有一个带有钱币配饰的黄金块】",
  },
  {
    batchIndex: 37,
    text: "$7 rare 普通和稀有视为同一种稀有度；史诗和传说视为同一种稀有度；【稀有度升级会同时作用于2种稀有度；解锁前提：向字母库中添加至少3种不同稀有度的字母块】",
  },
  {
    batchIndex: 38,
    text: "$4 common 你的单词的第一个字母会额外触发2次计分【解锁前提：使用全部由普通稀有度组成的单词击败boss】",
  },
  { batchIndex: 39, text: "$7 rare 传说字母在计分时会提供$10 【解锁前提：在字母库中拥有8个传说字母】" },
  { batchIndex: 40, text: "$7 rare 史诗字母在计分时提供x1.5倍率【解锁前提：在字母库中拥有16个史诗字母】" },
  { batchIndex: 41, text: "$7 rare 稀有字母在计分时提供+50分数【解锁前提：字母库中一半或以上的字母都是稀有字母】" },
  { batchIndex: 42, text: "$7 rare 普通字母在计分时提供+10倍率【解锁前提：字母库中所有字母都是普通字母】" },
  {
    batchIndex: 43,
    text: "$6 rare 每当一个碎冰块碎裂时，获得x0.75倍率（当前x1）【解锁前提：在字母库种拥有5个碎冰块】",
  },
