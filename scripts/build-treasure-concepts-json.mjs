...
const USER_BATCH_50 = [
  ...
  { batchIndex: 10, text: "$6 rare 如果拼写的单词为动词且以ify结尾，打开一个法术组合包" },
  { batchIndex: 11, text: "$6 rare 如果拼写的单词为形容词且以ous结尾，打开一个宝藏组合包" },
  { batchIndex: 12, text: "$6 rare 如果拼写的单词为名词且以ment结尾，打开一个升级组合包" },
  { batchIndex: 13, text: "$5 rare 如果拼写的单词为副词，打开一个字母组合包" },
  ...
  { batchIndex: 44, text: "$6 rare 如果你的一次拼写中拥有4种不同的稀有度，具有x3倍率【解锁前提：进入无尽模式】" },
  ...
  {
    batchIndex: 50,
    text: "$7 rare 当你触发boss的限制时，获得$8【如果是限制某种单词、例如只能是动词但玩家拼了名词。或者不能以普通字母结尾但玩家以普通字母结尾了。这种就算是触发。这个针对每种boss的效果不一样，你可以先帮我构思一下，对于每种boss玩家分别怎么样才算是触发】",
  },
];
